...
vector<ll> trial_division2(ll);

ll cnt = 0;
set<ll> c;
void f(vector<pair<ll,ll>> &v,ll k, ll i, ll ans, ll &n){
	if(i == sz(v)){
		// debug(ans)
		if(_gcd(ans,n-ans)+lcm(ans,n-ans) == n and ans!=n){
			// debug(ans)
			c.insert(ans);
		}
		return;
...
void solve2(){
    ...
    // for(ll i = 1;i<n;i++){
    // 	// debug(mp(i,n-i))
    // 	if(_gcd(i,n-i)+lcm(i,n-i) == (n)){
    // 		    	debug(mp(i,n-i))
    // 		ans++;
    // 	}
    // }
    map<ll,ll> mn;
    vector<ll> s = trial_division2(n);
    for(auto &it:s)
    {
    	mn[it]++;
    	// cout<<it<<" ";
    ...
    debug(c)
    ll g = 2*(sz(c));
    if(ceil((lld)log2(n)) == floor((lld)log2(n)) or n/2+n/2 == n){
    	g--;
    }
    // cout<<ans<<endl;
    cout<<g<<endl;
