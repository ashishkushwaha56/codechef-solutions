...
void solve(){
    ...
    */
    ...
    // k =9 n = 3;
    if(((1ll<<(pre))-1)*n == k){
        cout<<n*setbits((1ll<<(pre))-1)<<endl;
        return;
    }
    ll a = (1ll<<(pre))-1; // 7
    ll b = (1ll<<i)-1; // 15
    ll y = ceil((lld)abs(k-n*a)/(b-a)); // ceil(abs(12-2*7)/8) y = 1;
    ll x = n-y; // x = 1 
    ll new_t = y*b+x*a; // 1*15 + 1*7 = 22
    ll extra = new_t-k; // 10
    ans = setbits(b)*y+setbits(a)*x-setbits(extra); // set(b)*y = 4*1+set(a)*1 = 3*1 = 7 - set(10) = 5
    cout<<ans<<endl;
